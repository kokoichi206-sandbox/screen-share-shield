...
- **トリガ（すべて ~1秒 debounce）**: SPA 遷移（`pushState`/`replaceState`/`popstate`/`hashchange`）、
  `pageshow`、可視化、大きな DOM 変化（MutationObserver）、ユーザー操作（click/focus）。
- **共有中のときだけ**実行（自タブ共有 or background が live と通知した被共有タブ）かつ**可視時のみ**。
  非共有の通常ブラウジングでは Nano を起動しない。
- **DOM スナップショットが前回と変わった時だけ** Nano を呼ぶ（無駄打ち防止）。連続変化でも
  `maxWait` で最大数秒に1回は発火（starvation 防止）。検知結果は id 照合で最新のみ採用（順序逆転防止）。
- **残存リスク（fail-open の窓）**: 機密が描画されてから再検知が完了するまでの数秒は未マスク。
  機密が「ユーザー操作後に表示される」タイプならギャップは小さいが、初期HTMLにインラインで
  機密が出るページでは見えうる。確実に隠したい瞬間は手動「今すぐ検知」か `Ctrl+Shift+K`(全面)を使う。
...
- 自動再検知は「描画 → 再検知完了」の数秒に fail-open の窓が残る（初期HTMLインライン機密で顕著）。
