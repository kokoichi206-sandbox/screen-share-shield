...
- **MAIN world の宿命**: 差し替えに MAIN が必須ゆえ inject ↔ bridge の `postMessage` はページ JS から観測・偽造
  され得る。ただし inject が出すのは DOM スナップショットのみで、ページは元々自分の中身に full access を持つので
  新規漏洩ではない（検知用フレームは background の `captureVisibleTab` で取り postMessage には載らない）。新たに
  増える流れは「共有される側の rect 座標が共有元へ渡る」点のみ（位置情報のみ・低リスク）。完全隔離は不成立ゆえ受容。
